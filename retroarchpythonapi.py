...
class RetroArchPythonApi(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.logger.debug('Start in Fullscreen: %s' % self.settings['fullscreen'])
        self.logger.debug('Resolution: %s' % self.settings['resolution'])
        self.logger.debug('RetroArch Path: %s' % retroarch_path)
        self.logger.debug('Settings Path: %s' % self.pathes['settings'])
        self.logger.debug('Bios Path: %s' % self.pathes['bios'])
        self.logger.debug('Gamecontroller Path: %s' % self.pathes['controller'])
    ...
    def start(self, rom_path, core_path):
        ...
        cmd = [self.pathes['retroarch_path'], '--libretro', core_path, rom_path]#, '--config', self.pathes['configfile'], '--libretro', core_path, rom_path, '--size', self.settings['resolution'], '--verbose']
